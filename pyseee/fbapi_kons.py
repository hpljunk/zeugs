...
qstream = SELECT post_id, actor_id, target_id, message FROM stream WHERE source_id in (SELECT target_id FROM connection WHERE source_id=1056165140)
...
links of friends
qlinks = 'SELECT link_id,owner,owner_comment,created_time,title,summary,url,image_urls FROM link WHERE owner in (SELECT uid2 FROM friend WHERE uid1=1056165140)'
